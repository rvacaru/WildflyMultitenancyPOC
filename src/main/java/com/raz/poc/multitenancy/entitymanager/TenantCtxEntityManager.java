...
public class TenantCtxEntityManager implements EntityManager {
    ...
    private EntityManager entityManager() {
        ...
        Tenant tenant = (Tenant) ctxHolder.get("currentTenant");
        if (tenant == null){ 
            throw new RuntimeException("Tenant is unknown: not stored in Session ContextData");
        }
        
        em = (EntityManager) sessionCtx.lookup(tenant.getEntityManagerJndiName());

        if (em == null) {
            throw new RuntimeException("Could not find EntityManager in Session ContextData");
        }
        return em;
    }
}
